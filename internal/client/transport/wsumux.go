...
func NewWSUMuxClient(parentCtx context.Context, config *WsUMuxConfig, logger *logrus.Logger) *WsUMuxTransport {
	// Create a derived context from the parent context
	ctx, cancel := context.WithCancel(parentCtx)

	// Initialize the TcpTransport struct
	client := &WsUMuxTransport{
		smuxConfig: &smux.Config{
...
func (c *WsUMuxTransport) Restart() {
...
func (c *WsUMuxTransport) channelDialer() {
	...
	for {
		...
		default:
			...
			tunnelWSConn, err := WebSocketDialer(c.ctx, c.config.RemoteAddr, c.config.EdgeIP, "/channel", c.config.DialTimeOut, c.config.KeepAlive, true, c.config.Token, c.config.Mode, 3, 0, 0, 1320, "bbr", c.config.Travor)
			if err != nil {
...
func (c *WsUMuxTransport) poolMaintainer() {
...
func (c *WsUMuxTransport) channelHandler() {
...
func (c *WsUMuxTransport) tunnelDialer() {
	c.logger.Debugf("initiating new %s tunnel connection to address %s", c.config.Mode, c.config.RemoteAddr)

	// Dial to the tunnel server
	tunnelWSConn, err := WebSocketDialer(c.ctx, c.config.RemoteAddr, c.config.EdgeIP, "/tunnel", c.config.DialTimeOut, c.config.KeepAlive, c.config.Nodelay, c.config.Token, c.config.Mode, 3, 2*1024*1024, 2*1024*1024, 1320, "bbr", c.config.Travor)
	if err != nil {
...
func (c *WsUMuxTransport) handleSession(tunnelConn *websocket.Conn) {
	defer func() {
		atomic.AddInt32(&c.poolConnections, -1)
	}()
...
func (c *WsUMuxTransport) localDialer(stream *smux.Stream, remoteAddr string) {
